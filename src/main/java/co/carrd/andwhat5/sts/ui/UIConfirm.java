...
public class UIConfirm {
    ...
    public UIConfirm(Player player, EntityPixelmon pokemon, int price) {
        this.price = price;
    ...
    public void displayGUI() {
        ...
        Consumer<Action.Click> close = c -> {
            PlayerStorage playerPartyStorage = (PlayerStorage) PixelmonStorage.pokeBallManager.getPlayerStorage((EntityPlayerMP)this.player).orElse(null);
            UISTS ui = new UISTS(this.player, playerPartyStorage);
            ui.displayGUI();
        };
        Consumer<Action.Click> sell = s -> {
            if (Utilities.sellPokemon(this.player, this.pokemon, this.price)) {
                this.player.sendMessage(Text.of(("[STS] You sold your " + this.pokemon.getSpecies().name + " for $" + this.price + "!")));
                this.player.closeInventory();
            } else {
                this.player.sendMessage(Text.of(("[STS] Error selling " + this.pokemon.getSpecies().name + ". Is it still in your party?")));
                this.player.closeInventory();
            }
        };
        ItemStack rDye = ItemStack.builder()
                .itemType(ItemTypes.DYE)
                .add(Keys.DYE_COLOR, DyeColors.RED)
                .build();

        ItemStack gDye = ItemStack.builder()
                .itemType(ItemTypes.DYE)
                .add(Keys.DYE_COLOR, DyeColors.GREEN)
                .build();

        rDye.offer(Keys.DISPLAY_NAME, Text.of("\u2605 Cancel \u2605"));
        gDye.offer(Keys.DISPLAY_NAME, Text.of(("\u2605 Sell " + this.pokemon.getSpecies().name + " for $" + this.price + ". \u2605")));
        Element redDye = Element.of(rDye, close);
        Element greenDye = Element.of(gDye, sell);
        Layout layout = Layout.builder().set(blueGlass, 0, 1, 3, 4, 5, 7, 8).set(redGlass, 2, 6).set(blackGlass, 9, 17).set(whiteGlass, 13, 18, 19, 20, 21, 22, 23, 24, 25, 26).set(redDye, 12).set(greenDye, 14).build();
        View view = View.builder()
                .archetype(InventoryArchetypes.CHEST)
                .property(InventoryTitle.of(Text.of("Sell to Server - Confirm")))
